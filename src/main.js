  ...
  try {
    ...
    if (mobileCheck()) {
      ...
      window.addEventListener("devicemotion", (event) => {
        ...
        const absAxes = [Math.abs(x), Math.abs(y), Math.abs(z)];
        const overThreshold = absAxes.every(value => value >= threshold);

        if (overThreshold && !shaken) {
          world.rollDice();
          shaken = true;
        }

        else if (!overThreshold && shaken) {
          shaken = false;
        }
      });
    }
